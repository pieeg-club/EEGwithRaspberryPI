...
def graph (ch):
    ...
    if (fill_array==16):
        step=step+1  
        if (step == 8):           
                  data_for_shift_filter_1[ch] = data
        if (step == 16):
                  data_for_shift_filter_2[ch] = data
                  step=0

                  
        data_for_filter=data_for_shift_filter_1[ch-1]+data_for_shift_filter_2[ch]+data
        ...
        data_after_filter=data_band[500:750]
        #data_for_shift_filter[ch]=data
        return data_after_filter
    else:
        data_for_shift_filter[ch]=data
        data_for_shift_filter[ch-1] = data

        fill_array=fill_array+1
        data_emtpy_only_one_time=list(range(0,250,1))
...
data_for_shift_filter_1=([[1],[2],[3],[4],[5],[6],[7],[8]])
data_for_shift_filter_2=([[1],[2],[3],[4],[5],[6],[7],[8]])
