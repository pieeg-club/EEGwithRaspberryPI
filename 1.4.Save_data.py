...
libc = ctypes.CDLL("/home/pi/Desktop/EEGwithRaspberryPImaster/GUI/super_real_time_massive.so")
libc.prepare()
data_lenght_for_Filter = 4 # how much we read data for filter, all lenght [_____]
+ [_____] + [_____]
read_data_lenght_one_time = 1 # for one time how much read [_____]
lenght_data_for_filter = data_lenght_for_Filter*read_data_lenght_one_timeread_data_lenght_one_time
name_columns = []
final_dataset = []
...
while 1:
 ...
 libc.real.restype = ndpointer(dtype = ctypes.c_int,
shape=(sample_len,8))
 print ("ok1")
 ...
 axis[0].plot(range(axis_x,axis_x+sample_len,1),dataset[-250:],color =
'#0a0b0c')
 print ("ildar2")
 axis[0].axis([axis_x-x_minux_graph, axis_x+x_plus_graph, dataset[len(dataset)-
1]-y_minus_graph, dataset[len(dataset)-1]+y_plus_graph])
 print ("ildar3")
